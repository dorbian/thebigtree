...
function getOwnerClaimStatus(ownerName){
      ...
      // Media picker for dice/slots faces - uses existing uploadLibraryModal
      let diceMediaPickerTarget = null;

      function selectMediaForDice(item){
        if (!diceMediaPickerTarget) return;
        if (diceMediaPickerTarget === "dice_face"){
          $("diceFaceImageUrl").value = item.url || "";
        } else if (diceMediaPickerTarget === "dice_sprite_sheet"){
          $("diceSpriteSheetUrl").value = item.url || "";
        }
        showLibraryModal(false);
        diceMediaPickerTarget = null;
      }

      if ($("diceFaceLibrary")){
        $("diceFaceLibrary").addEventListener("click", () => {
          diceMediaPickerTarget = "dice_face";
          librarySelectHandler = selectMediaForDice;
          showLibraryModal(true);
          loadLibrary("media");
        });
      }

      if ($("diceSpriteSheetPicker")){
        $("diceSpriteSheetPicker").addEventListener("click", () => {
          diceMediaPickerTarget = "dice_sprite_sheet";
          librarySelectHandler = selectMediaForDice;
          showLibraryModal(true);
          loadLibrary("media");
        });
